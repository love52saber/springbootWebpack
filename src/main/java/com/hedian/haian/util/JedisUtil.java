...
//            logger.info(value);
    ...
    public static User getCurrentUser() {
        JedisUtil jedisUtil = ApplicationContextUtil.getBean(JedisUtil.class);
        ...
        String token = request.getHeader(CommonConstant.AUTH_TOKEN_KAY);
        if (token == null) {
            throw new UnauthorizedException("无授权信息");
        }
        String userInfoJsonStr = jedisUtil.get(String.format(CommonConstant.USER_TOKEN_PREX, token),
                CommonConstant.REDIS_INDEX);
        if (StringUtils.isEmpty(userInfoJsonStr)) {
            throw new UnauthorizedException("token过期");
        }
        User currentUser = JSON.parseObject(userInfoJsonStr, User.class);
        if (currentUser == null) {
            throw new BusinessException("查不到当前用户信息");
        }
        return currentUser;
    }
}
